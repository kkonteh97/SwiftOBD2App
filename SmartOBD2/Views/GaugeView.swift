...
//  SmartOBD2
//
//  Created by kemo konteh on 8/4/23.
//
...
struct GaugeView: View {
    ...
    func tickText(at tick: Int, text: String) -> some View {
        ...
        return Text(text).foregroundColor(colorMix(percent: percent)).rotationEffect(.init(degrees: -1 * rotation), anchor: .center).offset(x: -45, y: 0).rotationEffect(Angle.degrees(rotation))
    }
