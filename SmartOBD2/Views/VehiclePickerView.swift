        ...
        if viewModel.selectedYear != -1 && viewModel.selectedModel != -1 && viewModel.selectedManufacturer != -1 {
            Button(action: {
                viewModel.addVehicle(
                    make: viewModel.carData[viewModel.selectedManufacturer].make,
                    model: viewModel.models[viewModel.selectedModel].name,
                    year: String(viewModel.years[viewModel.selectedYear])
                )
            }) {
                Text("Add")
                    .padding()
